...
const DotField = memo(({
  ...
  useEffect(() => {
    ...
    let isVisible = true;
    let idleFrames = 0;
    const proofSection = document.querySelector(".landing-proof");

    function isPastProofSection() {
      if (!proofSection) {
        return false;
      }
      return proofSection.getBoundingClientRect().bottom <= 0;
    }
    ...
    function tick() {
      if (!isVisible || isPastProofSection()) {
        rafRef.current = requestAnimationFrame(tick);
        return;
      }
      ...
      const targetEngagement = mouseInBounds
        ? Math.max(Math.min(m.speed / 5, 1), 0.35)
